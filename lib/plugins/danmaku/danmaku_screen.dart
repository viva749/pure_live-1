...
import 'package:pure_live/plugins/danmaku/utils/utils.dart';
...
class _DanmakuScreenState extends State<DanmakuScreen> with TickerProviderStateMixin, WidgetsBindingObserver {
  ...
  void addDanmaku(DanmakuContentItem content) {
    ...
    if ((_scrollDanmakuItems.isNotEmpty || _topDanmakuItems.isNotEmpty || _bottomDanmakuItems.isNotEmpty) &&
  ...
  void _startTick() async {
    ...
    while (_running && mounted) {
      ...
      if (lastElapsedTime % 100 == 0) {
        ...
        // 移除顶部弹幕
        _topDanmakuItems.removeWhere((item) => ((_tick - item.creationTime) > (_option.duration * 1000)));
        // 移除底部弹幕
        _bottomDanmakuItems.removeWhere((item) => ((_tick - item.creationTime) > (_option.duration * 1000)));
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      _trackCount = (constraints.maxHeight * _option.area / _danmakuHeight).floor();
            ...
            child: Stack(children: [
              RepaintBoundary(
                ...
                builder: (context, child) {
                  return CustomPaint(
                    painter: ScrollDanmakuPainter(_animationController.value, _scrollDanmakuItems, _option.duration,
                        _option.fontSize, _option.fontWeight, _option.showStroke, _danmakuHeight, _running, _tick),
                    child: Container(),
                  );
                },
              )),
